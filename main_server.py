...
def chat_endpoint(request: ChatRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if request.image_base64:
            # Strip data URI prefix se existir (ex: data:image/jpeg;base64,)
            b64_data = request.image_base64
            mime_type = "image/jpeg" # default
            if "," in b64_data:
                prefix = b64_data.split(",", 1)[0]
                if ":" in prefix and ";" in prefix:
                    mime_type = prefix.split(":")[1].split(";")[0]
                b64_data = b64_data.split(",", 1)[1]
            
            # Decodifica de base64 string para raw bytes, pois a nova SDK exige bytes puros no data
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "429" in error_str or "quota" in error_str or "503" in error_str:
                            if attempt < max_retries - 1:
                                print(f"AVISO: Falha no modelo {current_model}. Alta demanda. Aguardando 5s...")
                                time.sleep(5) # Delay crucial para passar pelo gargalo 503
                                # Mantém o mesmo modelo e tenta de novo
                            else:
                                raise e
                        else:
                            raise e
...
async def whatsapp_webhook(request: __import__("fastapi").Request, background_tasks: BackgroundTasks):
    payload = await request.json()
    print("PAYLOAD_RECEIVED:", payload)
    """
